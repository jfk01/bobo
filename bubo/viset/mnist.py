...
def export(outdir=None):
    ...
    train_img_file = cache.get(TRAIN_IMG_URL)
    train_lbl_file = cache.get(TRAIN_LBL_URL)
    test_img_file = cache.get(TEST_IMG_URL)
    test_lbl_file = cache.get(TEST_LBL_URL)
    ...
    with open(outfile, 'wb') as csvfile:            
        ...
        with gzip.open(train_img_file, 'rb') as gzfile:
            ...
            for k in range(60000):
                ...
                imfile = os.path.join(cache.root(), 'im_%06d.png' % k)
        ...
        with gzip.open(test_img_file, 'rb') as gzfile:
            ...
            for k in range(10000):
                ...
                imfile = os.path.join(cache.root(), 'im_%06d.png' % (int(k)+60000))
                cv2.imwrite(imfile, im)
